...
function isValid(params) {
  ...
  if (
    !/^[a-zA-Z0-9_\-]+@([a-zA-Z0-9_\-]+\.)+[a-zA-Z]{2,4}$/.test(params["email"])
  )
    return false;
  return true;
}
...
function handleRegist(request, response, postData, database) {
  ...
  for (let i = 0; i < database.length; i++) {
    if (
      database[i]["name"] == params["name"] ||
      database[i]["id"] == params["id"] ||
      database[i]["email"] == params["email"] ||
      database[i]["phone"] == params["phone"]
    ) {
      isFound = 1;
      break;
    }
...
function start() {
  ...
  function onRequest(request, response) {
    ...
    request.addListener("end", function () {
      if (pathname == "/logIn")
        handleRegist(request, response, postData, database);
      postData = "";
    });
    if (pathname != "/logIn" && search == null) {
      let suffix = pathname.substr(
